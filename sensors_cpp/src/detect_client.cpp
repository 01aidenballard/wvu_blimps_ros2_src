...
#include "sensors_msgs/msg/joy.hpp"
#include "opencv/opencv.hpp"
#include <vector>
#include <thread>

class CamNode : publiv rclcpp: Node {
    public:
        ...
        void callback_read_image() {
            ...
            if (!control_mode) {
                ...
                int rows = frame.rows;
                int cols = frame.cols;
                std::vector<uchar> frameVector;
                if (frame.isContinuous()) {
                    frameVector.assign(frame.datastart, frame.dataend)
                } else {
                    for (int i = 0; i < frame.rows; ++i) {
                        frameVector.insert(frameVector.end(), frame.ptr<uchar>(i), frame.ptr<uchar>(i) + frame.cols * frame.elemSize());
                    }
                }
        ...
        void call_balloon_detection_service(std::vector convertedFrame) {
        ...
        void call_goal_detection_service(std::vector convertedFrame) {
