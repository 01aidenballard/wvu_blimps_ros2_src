...
from blimp_interfaces.msg import EscInputRudolph	#custom Interface for Esc inputs
...
class BaroNode(Node):
	def __init__(self):
		...
		self.publisher = self.create_publisher(EscInputRudolph, "ESC_Baro_input", 10)
	...
	def callback_baro_control(self,msg):
		...
		if height < self.height_goal:
			...
			msg2.pwm_sm2 = 1050.0
			msg2.pwm_sm3 = 1050.0
			msg2.pwm_vm = 1500.0 + abs(height-self.height_goal)*self.kpb
		...
		self.get_logger().info("baro: " + str(height)+ " U: "+ str(msg2.pwm_vm))
